...
The semantics of this GCL program is to nondeterministically choose either `j
...
I do find that this version of GCL has some unnecessary complexity. I prefer a
more algebraic presentation of the same language.
...
precondition, since `3 + 1 > 3`, however it is not the weakest, since `x = 3 ||
x = 4` also is a valid precondition and `x = 3 => x = 3 || x = 4`. The fourth condition
_is_ the weakest precondition. Notice that `x = 3 => x > 2` and `x = 3 || x = 4 =>
x > 2`. 
...
The rule for sequential composition simply propagates the precondition backwards through the program:

```
wlp (c1;c2, Q) = wlp(c1, wlp(c2, Q))
```

The final case is nondeterministic choice:

```
wlp (c1 [] c2, Q) = wlp(c1, Q) && wlp(c2, Q)
```

Intuitively, `[]` is demonic choice: at runtime we do not control whether `c1`
or `c2` is picked. So to guarantee `Q` after the choice, the input state must be
safe for _both branches_, not just one, so we join the WLPs for both `c1` and `c2`
w.r.t `Q`.
...
Now that we have the weakest precondition it's quite easy to build a symbolic
compiler for GCL programs. A symbolic compiler simply produces a formula φ that
describes the relational semantics of the input program. 
